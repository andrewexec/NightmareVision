...
class NativeAudioSource
{
	...
	inline function get_handle() return source;
	
	inline function set_handle(v) return source = v;
	
	var timer(get, set):Timer;
	
	inline function get_timer() return completeTimer;
	
	inline function set_timer(v) return completeTimer = v;
	
	var length(get, set):Null<Float>;
	
	inline function get_length() return endTime;
	
	inline function set_length(v) return endTime = v;
	
	var toLoop(get, set):Int;
	
	inline function get_toLoop() return streamLoops;
	
	inline function set_toLoop(v) return streamLoops = v;
	
	var bufferSizes(get, set):Array<Int>;
	
	inline function get_bufferSizes() return bufferLengths;
	
	inline function set_bufferSizes(v) return bufferLengths = v;
	...
	public function dispose()
	{
		...
		if (bufferDatas != null)
		{
			for (data in bufferDatas)
				if (bufferDataPool.length < POOL_MAX_BUFFERS) bufferDataPool.push(data);
			bufferDatas = null;
		}
	...
	public function resetBuffer()
	{
		...
		else
		{
			...
			if (audioBuffer.__srcBuffer != null)
			{
				...
				else buffer = audioBuffer.__srcBuffer;
			}
	...
	inline function streamSeek(samples:Int64) if (samples <= 1) parent.buffer.__srcVorbisFile.rawSeek(0);
	else parent.buffer.__srcVorbisFile.pcmSeek(samples);
	
	inline function streamTell():Int64 return parent.buffer.__srcVorbisFile.pcmTell();
	
	inline function streamRead(buffer:ArrayBuffer, position:Int, length:Int, wordSize:Int):Int return parent.buffer.__srcVorbisFile.read(buffer, position, length, isBigEndian, wordSize, true);
	...
	static function streamUpdate()
	{
		...
		{
			wasEmpty = false;
			if (threadRunning || (threadRunning = (streamThread = Thread.create(streamThreadRun)) != null)) streamThread.sendMessage(1);
		}
		#end
	...
	public function getCurrentTime():Float
	{
		...
		if (streamed)
		{
			...
			else if (bufferTimes != null) time += bufferTimes[STREAM_MAX_BUFFERS - requestBuffers];
		}
		time *= 1000;
